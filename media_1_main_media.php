...
?>
<div id="body1_m1">
<div style="width:100%;height:40px;margin-top:0px;padding-top:6px;background-color:#fcfcfc;border-bottom:1px solid #e2e2e2;margin-bottom:12px;padding-left:6px;">
	<div style="height:31px;float:left">
		<div style="display:table-cell;height:20px;padding-top:4px;vertical-align:top">
			<div style="display:inline-block;background-color:teal;width:5px;height:9px;margin-left:8px;margin-top:6px"></div><div style="display:inline-block;background-color:teal;width:5px;height:14px;margin-left:1px;margin-top:6px"></div><div style="display:inline-block;background-color:teal;width:5px;height:6px;margin-left:1px;margin-right:3px;margin-top:6px"></div>
		</div>
...
<? } ?>
<a class="newslist_a" href='media.php?media=<?=urlencode($media_name);?>' id="newsitem<?=$count?>"><div class="article_list_media" t="<?=$t;?>" articleno="<?=$articleno;?>" categoryno="<?=$category;?>" belong="<?=$media;?>" author="<?=$author;?>" mediano="<?=$media_no;?>" authorno="<?=$author_no;?>" chartid="<?=$articleno."_1_".$t;?>" dateint="<?=$article_date_int;?>">
	<div class="article_info_10" style="background: none; width:24%;">
		...
		<div style="border-radius:70px 70px 0% 0%;border:1px solid #D4D4D4;border-bottom:0px;width:70px;height:35px;margin:3px 5px 0px 20px;text-align:center">
			<div style="width:100%;height:100%;text-align:center;"><div style="width:100%;margin-top:6px;font-style:italic;margin-left:-11px;font-size:14px;color:#2a2a2a;"><?
	...
	<div class="article_info_12" style="width:43%;margin:0px;border:0px solid pink;vertical-align:top;">
		<div class="newslist_newsbox_in" style="width:100%;float:left;white-space:nowrap;overflow: hidden;text-overflow:ellipsis;padding:0px;margin-left:10px;">
				<div class="newslist_newsbox_in_in" style="margin-top:4px;"> 
					...
					<h4 class="newslist_title_h4" style="font-size:16px;line-height:1.32;margin-bottom:3px;"><?
							echo $media_name;
							?></h4><div style="width:100%;height:15px;font-size:11px;color:#9a9a9a;white-space:nowrap;overflow: hidden;text-overflow:ellipsis;margin-top:4px;"><?=$media_url;?></div>
					</div>
					<div class="newslist_badge" style="display:table-cell;vertical-align:top;height:14px;">
						<div style="display:inline-block;padding-top:0px;border:0px solid blue;height:9px;float:left;margin-right:3px;"><img src="img/ico_fb.png" style="width:9px;height:9px;margin: 0px 2px 0px 0px;"><div style="display:inline-block;padding-top:0px;border:0px solid blue;height:9px;float:right"><?=number_format($facebook_no);?></div></div>
						<div id="rated_time_<?=$t;?>_<?=$articleno;?>" class="rated_time" style="display:inline-block;padding-top:0px;border:0px solid blue;height:9px;float:left"><img src="img/ico_str.png" style="width:9px;height:9px;margin: 0px 2px 0px 0px;"><div style="display:inline-block;padding-top:0px;border:0px solid blue;height:9px;float:right"><?=number_format($rate_howmany);?></div></div>
					</div>
				</div>
		</div>
	</div>
...
var svgcont1_<?=$i;?> = d3.select("#media_chart_1_<?=$i;?>").append("svg")
...
d3.json("../rssdb/data_media_eval.php?media_no=<?=$media_no;?>&dateint=<?=$date_int;?>&periodno=<?=$periodno;?>", function(dataset){
	...
	// add legend   
	var legend<?=$i;?> = svgcont1_<?=$i;?>.append("g")
	.attr("class", "legend")
	.attr("x", width2)
	.attr("y", 50)
	.attr("height", height2)
	...
	var value<?=$i;?> = svgcont1_<?=$i;?>.append("g")
	.attr("class", "legend")
	.attr("x", width2)
	.attr("y", 50)
	.attr("height", height2)
...
<div style="margin:20px 30px 20px 30px;font-size:11px;line-height:160%;">통계상의 유효성을 위해 <?=$minimum_count;?>회 이상 평가가 누적된 언론사만 포함되었습니다.</div>
</div>

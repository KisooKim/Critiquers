...
<div style="width:100%;height:40px;margin-top:0px;padding-top:6px;background-color:#fcfcfc;border-bottom:1px solid #e2e2e2;margin-bottom:12px;padding-left:6px;">
	<div style="height:31px;float:left">
		...
		<div style="display:table-cell;height:20px;font-weight:bold;padding-top:9px;vertical-align:top">기자 순위 (<?=$date_int;?> 이전 <?=$period_string_array[$periodno];?>)</div>
	</div>
</div>
<?
$count=1;
...
<? } ?>
<a class="newslist_a" href='media.php?media=<?=urlencode($media_name);?>&author=<?=urlencode($author_name);?>' id="newsitem<?=$count?>"><div class="article_list_media" data-t="<?=$t;?>" data-articleno="<?=$articleno;?>" data-categoryno="<?=$category;?>" data-belong="<?=$media;?>" data-author="<?=$author;?>" data-mediano="<?=$media_no;?>" data-authorno="<?=$author_no;?>" data-chartid="<?=$articleno."_1_".$t;?>" data-dateint="<?=$article_date_int;?>">
		...
		<div class="newslist_newsbox_in" style="width:100%;float:left;white-space:nowrap;overflow: hidden;text-overflow:ellipsis;padding:0px;margin-left:10px;">
				<div class="newslist_newsbox_in_in" style="margin-top:4px;"> 
					...
					<h4 class="newslist_title_h4" style="font-size:16px;line-height:1.32;margin-bottom:3px;"><?
							echo $author_name;
							?></h4><div style="width:100%;height:15px;font-size:11px;color:#9a9a9a;white-space:nowrap;overflow: hidden;text-overflow:ellipsis;margin-top:4px;"><?=$media_name;?></div>
					</div>
...
var svgcont2_<?=$i;?> = d3.select("#media_chart_2_<?=$i;?>").append("svg")
...
d3.json("../rssdb/data_author_eval.php?author_no=<?=$author_no;?>&dateint=<?=$date_int;?>&periodno=<?=$periodno;?>", function(dataset){
	...
	// add legend   
	var legend<?=$i;?> = svgcont2_<?=$i;?>.append("g")
	.attr("class", "legend")
	.attr("x", width2)
	.attr("y", 50)
	.attr("height", height2)
	...
	var value<?=$i;?> = svgcont2_<?=$i;?>.append("g")
	.attr("class", "legend")
	.attr("x", width2)
	.attr("y", 50)
	.attr("height", height2)
...
<div style="margin:20px 30px 10px 30px;font-size:11px;line-height:160%;">저자의 이름이 함께 수집되는 기사에 한해 저자에 대한 평가가 누적됩니다.</div>

<div style="margin:10px 30px 20px 30px;font-size:11px;line-height:160%;">통계상의 유효성을 위해 <?=$minimum_count;?>회 이상 평가가 누적된 글쓴이만 포함되었습니다.</div>
